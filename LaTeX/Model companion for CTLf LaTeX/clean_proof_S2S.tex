...
\begin{definition}\label{CTLf_formulas}
    Let $\bar{p}= \{p_1,\ldots,p_n \}$ be a finite set of propositional variables. We define inductively a \emph{$CTL^{f}$-formula} $\varphi$ to be of the shape:
    \begin{itemize}
...
\begin{proof}
    ...
    \end{itemize}
\end{proof}
Note that $D_n(a,b)$ is the set: \[\{s\in S\mid \exists s_1,\ldots s_n, s=s_0Rs_1R\ldots Rs_n, s_k\in a \mbox{ for all } k<n \mbox{, and } s_n \in b\}\]
